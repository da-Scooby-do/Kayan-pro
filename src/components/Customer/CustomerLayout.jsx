...
const { user, mySession, rooms, profile } = useKayanStore(s => ({
  ...
  // cart count computed directly to avoid stale function ref
  const cart  = useKayanStore(s => s.cart)
  const count = cart.reduce((s, i) => s + i.qty, 0)

  // ── 1. Load all initial data on mount ──────────────────────
  useEffect(() => {
    if (!user?.id) return
    setBootstrapDone(false)
    bootstrapCustomer(user.id).finally(() => setBootstrapDone(true))
  }, [user?.id]) // eslint-disable-line

  // ── 2. Watch for admin opening/closing THIS customer's session
  //       This is what makes the session appear without refresh
  useCustomerSessionWatch(user?.id)

  // ── 3. Watch for order status updates (Pending→Ready)
...
{profile?.username && (
  <p className="text-[9px] text-kayan-muted font-mono mt-0.5">
    {profile.username}
  </p>
)}
          {profile?.username && (
