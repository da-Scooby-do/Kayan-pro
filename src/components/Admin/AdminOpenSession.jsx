...
//   1. Search by name/phone (original behaviour)
//   2. Enter an invitation code generated by the subscriber
function InviterPicker({ inviterId, inviterInfo, customers, onPickInviter }) {
  const { lookupInviteCode } = useKayan()
  ...
  const toggle = () => {
    if (open) { onPickInviter(null); setSearch(''); setCodeInput(''); setCodeStatus(null) }
    setOpen(v => !v)
  }
  ...
  const q = search.trim().toLowerCase()
  const filtered = open && !inviterId && mode === 'name'
    ? customers.filter(c =>
        c.full_name?.toLowerCase().includes(q) ||
        c.phone?.toLowerCase().includes(q)
      ).slice(0, 8)
  ...
  const handleCodeChange = (e) => {
    ...
    debounceRef.current = setTimeout(async () => {
      ...
      setCodeStatus('found')
      // Auto-resolve the inviter — build a "customer-like" object from the profile
      const inviterCustomer = { ...result.inviter, sub_status: 'active' }
      setLoadingInfo(true)
      await onPickInviter(inviterCustomer)
      setLoadingInfo(false)
    }, 500)
  }
        ...
        <div className="mt-3 space-y-3">
              ...
              <div className="flex rounded-xl overflow-hidden border border-white/[0.07] text-xs">
                {[['name', '🔍 By name'], ['code', '✦ By code']].map(([m, label]) => (
                  <button
                    key={m}
                    type="button"
                    onClick={() => { setMode(m); setSearch(''); setCodeInput(''); setCodeStatus(null) }}
                    className={`flex-1 py-2 font-medium transition-colors cursor-pointer border-none
                      ${mode === m
              ...
              {mode === 'code' && (
                <div className="space-y-2">
                  <div className="relative">
                    <input
                      type="text"
                      placeholder="Enter code e.g. A3F7C2D1"
                      value={codeInput}
                      onChange={handleCodeChange}
                      maxLength={8}
                      className="kayan-input text-sm font-mono tracking-widest uppercase pr-10"
                      autoFocus
                    />
                    {/* Status indicator */}
                    <div className="absolute right-3 top-1/2 -translate-y-1/2 text-sm">
                      {codeStatus === 'looking' && (
                        <span className="text-kayan-muted animate-pulse">…</span>
                      )}
                      {codeStatus === 'found' && (
                  ...
                  {codeStatus === 'looking' && (
                    <p className="text-[10px] text-kayan-muted animate-pulse">Looking up code…</p>
                  )}
                </div>
              )}
            </>
          ...
          {inviterId && (
            <div className={`rounded-xl p-3 border ${
              ...
              {loadingInfo ? (
                <p className="text-[10px] text-kayan-muted animate-pulse">Checking invitations…</p>
              ) : noInv ? (
                <>
                ...
                <>
                  <div className="flex items-center gap-2 mb-1">
                    <span className="text-xs font-semibold text-kayan-gold">✠ Invitation Pass</span>
                    {mode === 'code' && (
                      <span className="text-[8px] bg-kayan-gold/15 text-kayan-gold border border-kayan-gold/30 px-1.5 py-0.5 rounded-full">
                        via code
                      </span>
                    )}
                  </div>
                  <p className="text-[10px] text-kayan-muted">
                    {remaining !== null
                      ...
                      : 'Checking…'}
                    {' '}— stay will be{' '}
                    <strong className="text-kayan-text">free</strong>; orders only.
                  </p>
                </>
              )}
              <button
